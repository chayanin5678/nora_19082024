    ...
    <div class="<?php if (wp_traveler_is_active_sidebar("sidebar_right") && wp_traveler_is_active_sidebar("sidebar_left")) {
      ...
      <header class="archive-header">
        <h1 class="archive-title">
          <?php
          if (is_day()):
            printf(__('Daily Archives: %s', 'wp-traveler'), get_the_date());
          elseif (is_month()):
            printf(__('Monthly Archives: %s', 'wp-traveler'), get_the_date(_x('F Y', 'monthly archives date format', 'wp-traveler')));
          elseif (is_year()):
            printf(__('Yearly Archives: %s', 'wp-traveler'), get_the_date(_x('Y', 'yearly archives date format', 'wp-traveler')));
          else:
            _e('Archives', 'wp-traveler');
          endif;
          ?>
        </h1>
